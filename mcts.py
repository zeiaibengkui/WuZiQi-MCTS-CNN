...
class MCTS:
    ...
    def search(self, game_state):
        ...
        for _ in range(self.n_simulations):
            ...
            if not game_copy.is_terminal():
                        ...
                        for center_r, center_c in [(last_r, last_c), (prev_r, prev_c)]:
                            for dr in range(-radius, radius + 1):
                                for dc in range(-radius, radius + 1):
                                    ...
                                    if 0 <= nr < config.BOARD_SIZE and 0 <= nc < config.BOARD_SIZE and game_copy.board[nr, nc] == 0:
                                        focused.add((nr, nc))
                ...
                # game over
                value = 1.0 if game_copy.winner == game_copy.current_player else -1.0  # note perspective
    ...
    def get_action_distribution(self, game_state, temperature=1):
        # priority move from game's critical moves (human vs AI will also go through this)
        priority_move = None  # game_state.get_priority_move()  # commented out in original
